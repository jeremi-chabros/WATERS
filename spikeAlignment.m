...
for i = 1:length(spikeTimes)
    ...
    if (spikePoint + round(duration/2)) > length(data)
        surrogate = zeros(1,251);
        ending = length(data(spikePoint - round(duration/2):end));
        surrogate(1:ending) = data(spikePoint - round(duration/2):end);
        spikeStore(i, :) = surrogate;
    else
